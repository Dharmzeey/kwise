...
function buildDjangoUrl(segments: string[], search: string): string {
  ...
  return `${DJANGO_URL}/api/swap/${clean.join("/")}/` + (search ?? "");
}
...
async function proxy(djangoUrl: string, init?: RequestInit): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[swap proxy] Django unreachable:", djangoUrl, err);
  ...
  try {
    ...
    console.error("[swap proxy] Non-JSON response from Django:", res.status, text.slice(0, 200));
    return NextResponse.json(
