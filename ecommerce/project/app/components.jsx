/* Kwise World — shared components. Exports to window. */
...
function Thumb({ product, size = "card" }) {
  const tint = product.tint || "blue";
  return (
    <div className={"thumb thumb-" + tint + " thumb-" + size} role="img"
      aria-label={product.name + " photo placeholder"}>
      <div className="thumb-glow" />
      <Icon name={product.thumb || "phone"} size={size === "hero" ? 120 : size === "lg" ? 96 : 64} stroke={1.2} className="thumb-icon" />
...
function Stars({ value = 0, size = 14, showNum = false, count }) {
  ...
  return (
    <span className="stars">
      ...
      {showNum && <span className="stars-num">{value.toFixed(1)}{count != null ? ` (${count})` : ""}</span>}
    </span>
  );
}
...
function StatusPill({ status }) {
  ...
  return <span className={"status-pill st-" + (map[status] || "new")}>{status}</span>;
}

/* ---------------- Product card ---------------- */
function ProductCard({ product, onNav, onAdd }) {
  const { formatNaira } = window.KW;
  const soldOut = product.oneTime && product.stock <= 0;
  return (
    <article className={"pcard" + (product.oneTime ? " pcard-ot" : "")}>
      <button className="pcard-media" onClick={() => onNav("product", product.id)} aria-label={"View " + product.name}>
        <Thumb product={product} />
        <div className="pcard-badges">
          {product.oneTime && <span className="badge badge-ot"><Icon name="bolt" size={12} stroke={0} /> One-Time</span>}
          {product.badge && !product.oneTime && <span className="badge badge-feat">{product.badge}</span>}
          {product.oldPrice && <span className="badge badge-save">Save {formatNaira(product.oldPrice - product.price)}</span>}
        </div>
        {soldOut && <div className="pcard-sold"><span>Sold Out</span></div>}
      </button>
      <div className="pcard-body">
        ...
        <h3 className="pcard-name" onClick={() => onNav("product", product.id)}>{product.name}</h3>
        <div className="pcard-foot">
          ...
          <button className="btn-add" disabled={soldOut} onClick={() => onAdd(product)} aria-label={"Add " + product.name + " to cart"}>
            <Icon name={soldOut ? "close" : "plus"} size={18} />
          </button>
        </div>
        {product.oneTime && product.stock > 0 && <div className="pcard-ot-note"><Icon name="bolt" size={12} stroke={0} /> Only 1 available</div>}
      </div>
    </article>
  );
}
...
/* ---------------- Buttons ---------------- */
function Btn({ children, kind = "primary", size = "md", icon, iconAfter, onClick, href, type, disabled, full, className = "" }) {
  const cls = `btn btn-${kind} btn-${size}${full ? " btn-full" : ""} ${className}`;
  const content = (
    <>
      {icon && <Icon name={icon} size={size === "lg" ? 20 : 17} />}
      {children && <span>{children}</span>}
      {iconAfter && <Icon name={iconAfter} size={size === "lg" ? 20 : 17} />}
    </>
  );
  if (href !== undefined)
    return <a className={cls} onClick={onClick} role="button">{content}</a>;
  return <button className={cls} type={type || "button"} onClick={onClick} disabled={disabled}>{content}</button>;
}

/* ---------------- Header ---------------- */
function Header({ route, onNav, cartCount, onOpenCart, search, setSearch }) {
  ...
  useEffect(() => { setOpen(false); }, [route.page, route.param]);

  const submit = (e) => {
    e.preventDefault();
    setSearch(q);
    onNav("category", "all", { q });
  };
    ...
    <>
      <div className="topbar">
        <div className="container topbar-in">
          <span><Icon name="truck" size={15} /> Free delivery in Lagos over ₦500,000</span>
          <span className="topbar-tag">Integrity — we mean what we say.</span>
          <a className="topbar-wa" onClick={() => onNav("about")}><Icon name="whatsapp" size={15} /> Chat with us</a>
        </div>
      </div>
      <header className="header">
        <div className="container header-in">
          <button className="hamburger" onClick={() => setOpen(true)} aria-label="Open menu"><Icon name="menu" /></button>
          ...
          <nav className="mainnav" onMouseLeave={() => setMegaOpen(false)}>
            <button className="navlink" onMouseEnter={() => setMegaOpen(true)} onClick={() => setMegaOpen(v => !v)}>
              Shop <Icon name="chevronDown" size={15} />
            </button>
            {navItems.map((c) => (
              <button key={c.slug} className="navlink" onClick={() => onNav("category", c.slug)}>{c.name}</button>
            ))}
            <button className="navlink navlink-ot" onClick={() => onNav("offers")}>
              <Icon name="bolt" size={14} stroke={0} /> One-Time Offers
            </button>
            <button className="navlink" onClick={() => onNav("about")}>About</button>

            {megaOpen && (
              <div className="mega" onMouseEnter={() => setMegaOpen(true)}>
                {CATEGORIES.map((c) => (
                  <div className="mega-col" key={c.slug}>
                    <button className="mega-head" onClick={() => onNav("category", c.slug)}>
                      <Icon name={c.icon} size={18} /> {c.name}
                    </button>
                    <div className="mega-links">
                      {c.brands.map((b) => (
                        <button key={b.slug} onClick={() => onNav("category", c.slug, { brand: b.slug })}>{b.name}</button>
                      ))}
                    </div>
                  </div>
                ))}
                <div className="mega-promo">
                  ...
                  <button onClick={() => onNav("offers")}>See deals <Icon name="arrowRight" size={15} /></button>
                </div>
              </div>
            )}
          </nav>
          <form className="searchbar" onSubmit={submit}>
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search phones, laptops, accessories…" />
          </form>
          <div className="header-actions">
            <button className="iconbtn" onClick={() => onNav("login")} aria-label="Account"><Icon name="user" /></button>
            <button className="iconbtn cartbtn" onClick={onOpenCart} aria-label="Cart">
              <Icon name="cart" />
      ...
      <div className={"mobile-menu" + (open ? " open" : "")}>
        <div className="mm-scrim" onClick={() => setOpen(false)} />
        <div className="mm-panel">
          <div className="mm-head">
            <span className="logo-text">Kwise<span> World</span></span>
            <button className="iconbtn" onClick={() => setOpen(false)}><Icon name="close" /></button>
          </div>
          <form className="searchbar mm-search" onSubmit={submit}>
            <Icon name="search" size={18} />
          ...
          <div className="mm-links">
            {CATEGORIES.map((c) => (
              <div key={c.slug} className="mm-group">
                <button className="mm-link mm-cat" onClick={() => onNav("category", c.slug)}><Icon name={c.icon} size={18} />{c.name}</button>
                <div className="mm-sub">
                  {c.brands.map((b) => <button key={b.slug} onClick={() => onNav("category", c.slug, { brand: b.slug })}>{b.name}</button>)}
                </div>
              </div>
            ))}
            <button className="mm-link mm-ot" onClick={() => onNav("offers")}><Icon name="bolt" size={18} stroke={0} />One-Time Offers</button>
            <button className="mm-link" onClick={() => onNav("about")}>About</button>
            <button className="mm-link" onClick={() => onNav("login")}><Icon name="user" size={18} />Login / Sign up</button>
          </div>
        </div>
      </div>
    </>
    ...
    <footer className="footer">
      <div className="container footer-in">
        <div className="footer-brand">
          ...
          <p className="footer-tag">Integrity — we mean what we say. Gadgets you can trust, prices that respect you.</p>
        ...
        <div className="footer-col">
          <h4>Shop</h4>
          {CATEGORIES.map((c) => <button key={c.slug} onClick={() => onNav("category", c.slug)}>{c.name}</button>)}
          <button onClick={() => onNav("offers")}>One-Time Offers</button>
        </div>
        <div className="footer-col">
          <h4>Company</h4>
          <button onClick={() => onNav("about")}>About us</button>
          <button onClick={() => onNav("about")}>Our promise</button>
          <button onClick={() => onNav("about")}>Contact</button>
          <button onClick={() => onNav("login")}>My account</button>
        </div>
        <div className="footer-col footer-contact">
          <h4>Reach us</h4>
          <span><Icon name="pin" size={16} /> Computer Village, Ikeja, Lagos</span>
          <span><Icon name="whatsapp" size={16} /> +234 800 000 0000</span>
...
Object.assign(window, { Icon, Thumb, Stars, StatusPill, ProductCard, Btn, Header, Footer });
